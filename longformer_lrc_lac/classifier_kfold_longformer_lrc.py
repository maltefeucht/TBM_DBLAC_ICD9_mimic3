...
from constants_mimic3 import MIMIC_3_DIR, MIMIC_3_DIR_VM, DATA_DIR, DATA_DIR_VM, PROJECT_DIR, PROJECT_DIR_VM
from inference_and_metrics import metrics, inference
from longformer_lrc_lac import datasetloader_mimic3_longformer as datasetloader_mimic3
...
class classifier(pl.LightningModule):
    ...
    def __init__(self):
        super().__init__()
        self.model = transformers.LongformerForSequenceClassification.from_pretrained('allenai/longformer-base-4096', gradient_checkpointing=True, num_labels = num_classes, return_dict=True)
    ...
    def forward(self, input_ids, attention_mask, global_attention_mask=None):
        ...
        """
        #model_summary = summary(self.model, input_size=(FLAGS.batch_size,4096))
        #print(model_summary)

        if global_attention_mask is None:
            global_attention_mask = th.zeros_like(input_ids)
...
def main(_):
    ...
    if FLAGS.inference == False:
        ...
        if FLAGS.training == True:
            ...
            for fold, (train_idx, val_idx) in enumerate(kfold.split(train_full_kfold_ds if FLAGS.Dataset_full else train_50_kfold_ds)):
                ...
                if FLAGS.local == True:
                    if FLAGS.Dataset_full == False:
                        logger = TensorBoardLogger("tb_logs", name="classifier_kfold_50_longformer_lrc_lm")
                        logger.log_hyperparams(
                            {'learning_rate': FLAGS.lr, 'epochs': FLAGS.epochs, 'batch_size': FLAGS.batch_size,
                             'num_labels': num_classes})
                    else:
                        logger = TensorBoardLogger("tb_logs", name="classifier_kfold_full_longformer_lrc_lm")
                        logger.log_hyperparams(
                            {'learning_rate': FLAGS.lr, 'epochs': FLAGS.epochs, 'batch_size': FLAGS.batch_size,
                             'num_labels': num_classes})
                else:
                    if FLAGS.Dataset_full == False:
                        logger = TensorBoardLogger("tb_logs", name="classifier_kfold_50_longformer_lrc_vm")
                        logger.log_hyperparams(
                            {'learning_rate': FLAGS.lr, 'epochs': FLAGS.epochs, 'batch_size': FLAGS.batch_size,
                             'num_labels': num_classes})
                    else:
            ...
            model = classifier()
            model.load_state_dict(th.load('%s/model_files/classifier_kfold_longformer_lrc_vm_01_08_2021_14:59:26.pth' % PROJECT_DIR if FLAGS.local else PROJECT_DIR_VM, map_location=th.device('cpu')))
    ...
    if FLAGS.inference == True:
        ...
        if FLAGS.Dataset_full:
            with open('%s/results/longformer_lrc/predictions_longformer_lrc_full.csv' % PROJECT_DIR if FLAGS.local else '%s/results/longformer_lrc/predictions_longformer_lrc_full.csv' % PROJECT_DIR_VM,'w') as outfile:
                columns = ['HADM_ID', 'TEXT', 'GROUND_TRUTH', 'PREDICTIONS']
                w = csv.DictWriter(outfile, fieldnames=columns)
                w.writeheader()
        else:
            with open(
                    '%s/results/longformer_lrc/predictions_longformer_lrc_50.csv' % PROJECT_DIR if FLAGS.local else '%s/results/longformer_lrc/predictions_longformer_lrc_50.csv' % PROJECT_DIR_VM,
                    'w') as outfile:
                columns = ['HADM_ID', 'TEXT', 'GROUND_TRUTH', 'PREDICTIONS']
                w = csv.DictWriter(outfile, fieldnames=columns)
                w.writeheader()
