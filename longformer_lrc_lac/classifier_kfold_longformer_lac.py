...
class classifier(pl.LightningModule):
    ...
    def __init__(self):
        ...
        if FLAGS.freeze_longformer == True:
            self.model = transformers.LongformerModel.from_pretrained('allenai/longformer-base-4096', output_hidden_states=True, return_dict=True)
            for p in self.model.parameters():
                p.requires_grad = False
        else:
            self.model = transformers.LongformerModel.from_pretrained('allenai/longformer-base-4096',gradient_checkpointing=True, output_hidden_states=True, return_dict=True)
    ...
    def forward(self, input_ids, attention_mask, global_attention_mask=None):
        ...
        """
        # Implement logic for moving tensor and classes to gpu if available
        ...
        # classifier
        outputs = th.zeros((weighted_output.size(0), num_classes))#.to(device)
...
def main(_):
    ...
    if FLAGS.inference == False:
        ...
        if FLAGS.training == True:
            ...
            for fold, (train_idx, val_idx) in enumerate(kfold.split(train_full_kfold_ds if FLAGS.Dataset_full else train_50_kfold_ds)):
                ...
                if FLAGS.local == True:
                    if FLAGS.Dataset_full == False:
                        logger = TensorBoardLogger("tb_logs", name="classifier_kfold_50_longformer_lac_lm")
                        logger.log_hyperparams(
                            {'learning_rate': FLAGS.lr, 'epochs': FLAGS.epochs, 'batch_size': FLAGS.batch_size,
                             'num_labels': num_classes})
                    else:
                        logger = TensorBoardLogger("tb_logs", name="classifier_kfold_full_longformer_lac_lm")
                        logger.log_hyperparams(
                            {'learning_rate': FLAGS.lr, 'epochs': FLAGS.epochs, 'batch_size': FLAGS.batch_size,
                             'num_labels': num_classes})
                else:
                    if FLAGS.Dataset_full == False:
                        logger = TensorBoardLogger("tb_logs", name="classifier_kfold_50_longformer_lac_vm_old")
                        logger.log_hyperparams(
                            {'learning_rate': FLAGS.lr, 'epochs': FLAGS.epochs, 'batch_size': FLAGS.batch_size,
                             'num_labels': num_classes})
                    else:
            ...
            model = classifier()
            model.load_state_dict(th.load('%s/model_files/classifier_kfold_50_longformer_lac_vm_26_08_2021_14:13:21.pth' % PROJECT_DIR if FLAGS.local else PROJECT_DIR_VM, map_location=th.device('cpu')))
    ...
    if FLAGS.inference == True:
        ...
        # instantiate inference dataloader -> batch_size setting to 1 important and not to be changed.
