...
class classifier(pl.LightningModule):
    ...
    def __init__(self):
        super().__init__()
        self.model = transformers.BertForSequenceClassification.from_pretrained('bert-base-uncased', gradient_checkpointing=True, num_labels = num_classes, return_dict=True)
...
def main(_):
    ...
    if FLAGS.inference == False:
        ...
        if FLAGS.training == True:
            ...
            for fold, (train_idx, val_idx) in enumerate(kfold.split(train_full_kfold_ds if FLAGS.Dataset_full else train_50_kfold_ds)):
                ...
                if FLAGS.local == True:
                    if FLAGS.Dataset_full == False:
                        logger = TensorBoardLogger("tb_logs", name="classifier_kfold_50_bert_lrc_lm")
                        logger.log_hyperparams(
                            {'learning_rate': FLAGS.lr, 'epochs': FLAGS.epochs, 'batch_size': FLAGS.batch_size,
                             'num_labels': num_classes})
                    else:
                        logger = TensorBoardLogger("tb_logs", name="classifier_kfold_full_bert_lrc_lm")
                        logger.log_hyperparams(
                            {'learning_rate': FLAGS.lr, 'epochs': FLAGS.epochs, 'batch_size': FLAGS.batch_size,
                             'num_labels': num_classes})
                else:
                    if FLAGS.Dataset_full == False:
                        logger = TensorBoardLogger("tb_logs", name="classifier_kfold_50_bert_lrc_vm")
                        logger.log_hyperparams(
                            {'learning_rate': FLAGS.lr, 'epochs': FLAGS.epochs, 'batch_size': FLAGS.batch_size,
                             'num_labels': num_classes})
                    else:
            ...
            model = classifier()
            model.load_state_dict(th.load('%s/model_files/classifier_kfold_bert_lrc_vm_27_07_2021_05:33:38.pth' % PROJECT_DIR if FLAGS.local else PROJECT_DIR_VM, map_location=th.device('cpu')))
    ...
    if FLAGS.inference == True:
        ...
        if FLAGS.Dataset_full:
            with open('%s/results/bert_lrc/predictions_bert_lrc_full.csv' % PROJECT_DIR if FLAGS.local else '%s/results/bert_lrc/predictions_bert_lrc_full.csv' % PROJECT_DIR_VM,'w') as outfile:
                columns = ['HADM_ID', 'TEXT', 'GROUND_TRUTH', 'PREDICTIONS']
                w = csv.DictWriter(outfile, fieldnames=columns)
                w.writeheader()
        else:
            with open(
                    '%s/results/bert_lrc/predictions_bert_lrc_50.csv' % PROJECT_DIR if FLAGS.local else '%s/results/bert_lrc/predictions_bert_lrc_50.csv' % PROJECT_DIR_VM,
                    'w') as outfile:
                columns = ['HADM_ID', 'TEXT', 'GROUND_TRUTH', 'PREDICTIONS']
                w = csv.DictWriter(outfile, fieldnames=columns)
                w.writeheader()
