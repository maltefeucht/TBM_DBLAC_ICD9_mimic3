...
def vocab_index_descriptions(vocab_file, vectors_file, vectors_file_raw, vectors_file_csv):
    # load lookups
    vocab = set()
    ...
    # save desc_dict
    np.save(vectors_file_raw, desc_dict)
    # save desc_dict as csv
    df = pd.DataFrame.from_dict(desc_dict, orient='index')
    df.to_csv(vectors_file_csv)
